...
import { vendorResponses, vendorEligibility } from '../db/schema';
import { eq, inArray } from 'drizzle-orm';
import { buildComparison } from './comparison';
export async function optimizeSplit(rfxId, lineItemIds, constraints = {}) {
    ...
    const feasibleVendorIds = new Set();
    if (constraints.requireFeasible) {
        ...
        for (const e of eligibility) {
            if (e.isEligible)
                feasibleVendorIds.add(e.vendorResponseId);
        }
    }
    else {
        for (const v of vendors)
            feasibleVendorIds.add(v.id);
    }
    if (constraints.excludedVendors) {
        for (const id of constraints.excludedVendors)
            feasibleVendorIds.delete(id);
    }
    const allocations = [];
    const vendorAllocations = new Map();
    const unallocated = [];
    for (const line of relevantLines) {
        const validQuotes = line.quotations.filter(q => q.pricePerBaseUnit !== null &&
...
//# sourceMappingURL=split-analysis.js.map